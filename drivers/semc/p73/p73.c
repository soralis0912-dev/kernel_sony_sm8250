...
struct p61_dev {
    ...
    /*
     * SPI transfer buffers. These must come from kmalloc, not the stack:
     * with VMAP_STACK the kernel stack lives in vmalloc space, and once a
     * transfer is long enough for the GENI controller to take its DMA path
     * (geni_se_rx_dma_prep -> dma_map_single) the address translation walks
     * off into an unmapped page and takes the kernel down:
     *
     *   Unable to handle kernel paging request  pc: __dma_inv_area
     *   geni_se_rx_dma_prep / spi_geni_do_transfer_one / __spi_sync / vfs_read
     *
     * Short reads stay in the FIFO path, which is why this only showed up
     * once the eSE started answering with full-size packets.
     */
        ...
        unsigned long arg)
{
    ...
    /* Same DMA-safety rule as p61_dev_read/write - see struct p61_dev. */
    unsigned char *buf;
    ...
    case P61_SET_PWR:
        ...
        else if (arg == 1)
        {
            ...
            msleep(50);
            ret = spi_read (p61_dev -> spi,(void *) buf, buf_len);
            msleep(50);
            gpio_set_value(p61_dev->rst_gpio, 1);
            msleep(20);
        ...
        loff_t *offset)
{
    ...
    unsigned char *rx_buffer = p61_dev->rx_buf;
...
#endif
    /*
     * No soft reset line on this platform: the eSE is reset over the NFC
     * controller (P61_SET_SPM_PWR -> pn553), so there is no GPIO to claim.
     */
    ret = 0;
    P61_DBG_MSG("Exit : %s\n", __FUNCTION__);
    return ret;
    ...
    struct p61_spi_platform_data *data)
{
    ...
    /*
     * The eSE hangs off the SPI bus with neither an interrupt nor a reset
     * line of its own - power and reset are driven by the NFC controller
     * through pn544_dev_ioctl(), and reads are done in polling mode. So
     * there is nothing to pick up from the device node here.
     */
    data->irq_gpio = 0;
    data->rst_gpio = 0;
...
static int p61_probe(struct spi_device *spi)
{
    ...
    /* kmalloc'd so the GENI DMA path can map them - see struct p61_dev */
    p61_dev->tx_buf = kzalloc(MAX_BUFFER_SIZE, GFP_KERNEL);
    p61_dev->rx_buf = kzalloc(MAX_BUFFER_SIZE, GFP_KERNEL);
    if (p61_dev->tx_buf == NULL || p61_dev->rx_buf == NULL)
    {
        P61_ERR_MSG("failed to allocate SPI transfer buffers\n");
        ret = -ENOMEM;
        goto err_exit0;
    }
    ...
    if(p61_dev != NULL)
    {
        kfree(p61_dev->tx_buf);
        kfree(p61_dev->rx_buf);
        kfree(p61_dev);
    }
    err_exit:
